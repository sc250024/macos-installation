...
class InMemoryZip(object):
    def __init__(self, file: t.Optional[pathlib.Path] = None):
        # Inputs
        self.file_path = file

        # Evaluated from inputs
        self.zip_contents: t.IO[bytes] = io.BytesIO(
            self.file_path.read_bytes() if self.file_path else None
        )

        logger.debug(
            "Class 'InMemoryZip' instantiated: "
            f"{pformat({k: v for k, v in self.__dict__.items()})}"
        )
    ...
    def write_to_file(self, filename: pathlib.Path) -> t.NoReturn:
        ...
        :return: None
        """
        filename.write_bytes(self.read())


class EncryptedZip(InMemoryZip):
    def __init__(
        ...
        logger.debug(
            "Class 'EncryptedZip' instantiated: "
            f"{pformat({k: v for k, v in self.__dict__.items()})}"
        )
